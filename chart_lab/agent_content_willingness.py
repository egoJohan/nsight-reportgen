#!/usr/bin/env python3
"""Build the willingness-to-choose / recommend slide (nSight house style).

Grouped horizontal bar chart: % of evaluators who could imagine choosing a
provider for themselves or would recommend it, across FOUR survey waves grouped
per provider, the current wave (Marraskuu 2025) emphasised, PLUS a
"change-vs-previous" column (Marraskuu 2025 - Toukokuu 2025) on the right.

Providers are ordered best->worst by the current wave. The opt-out option
"En mitään näistä" (none of these) is NOT a provider, so it is held out of the
provider ranking and shown last in a muted style, separated by a rule.

All numbers and deltas are derived deterministically from willingness.json.
Validia has no Toukokuu 2024 reading; that bar is simply not drawn (no value
is invented).
"""
import json
...
DATA = ROOT / "chart_lab" / "willingness.json"
PNG = ROOT / "chart_lab" / "agent_content_willingness_chart.png"
OUT = ROOT / "work" / "agent_content_willingness.pptx"
...
OPTOUT = "En mitään näistä"   # opt-out option, not a provider

# Order providers best->worst by the CURRENT wave; opt-out always last.
prov_idx = [i for i in range(len(cats)) if cats[i] != OPTOUT]
prov_idx.sort(key=lambda i: series[CURRENT][i], reverse=True)
optout_idx = [i for i in range(len(cats)) if cats[i] == OPTOUT]
order = prov_idx + optout_idx        # opt-out pinned to the bottom

cats_o = [cats[i] for i in order]
vals = {w: [series[w][i] for i in order] for w in WAVES}
is_optout = [cats[i] == OPTOUT for i in order]
...
# Muted greyscale palette for the opt-out row (de-emphasised, not a provider).
OPTOUT_COLORS = {
    "Toukokuu 2024":  "#DAD5CC",
    "Marraskuu 2024": "#C3BDB2",
    "Toukokuu 2025":  "#A7A096",
    "Marraskuu 2025": "#7C766C",
}

# ---------------------------------------------------------------- figure
FIG_W, FIG_H = 12.4, 6.25
...
# Left chart axes + a narrow change column on the right (shared y range).
ax = fig.add_axes([0.150, 0.085, 0.665, 0.875])
ax.set_facecolor(CREAM)
cax = fig.add_axes([0.840, 0.085, 0.150, 0.875])
cax.set_facecolor(CREAM)

XMAX = 50  # data tops out at 41

# --- bars
for wi, w in enumerate(WAVES):
    off = group_h / 2 - bar_h * (wi + 0.5)
    is_cur = (w == CURRENT)
    for ri, y in enumerate(ypos):
        x = vals[w][ri]
        if x is None:
            continue
        col = (OPTOUT_COLORS if is_optout[ri] else WAVE_COLORS)[w]
        ax.barh(y + off, x, height=bar_h * 0.94, color=col,
                edgecolor="none", zorder=3)
        ax.text(x + 0.6, y + off, f"{x}", va="center", ha="left",
                fontsize=7.8 if not is_cur else 8.8,
                fontweight="bold" if is_cur else "normal",
                color=INK if (is_cur and not is_optout[ri]) else MUTED, zorder=5)

# category labels
ax.set_yticks(ypos)
ax.set_yticklabels(cats_o, fontsize=11.5, color=INK)
LEADERS = {"Julkiset hoivapalvelut", "Attendo"}
...
# separator rule between the providers and the opt-out row
n_prov = len(prov_idx)
sep_y = (n - n_prov) - 0.5   # boundary below the last provider row
ax.axhline(sep_y, color="#C9C1B4", lw=1.0, ls=(0, (4, 3)), zorder=2)
...
for ri, y in enumerate(ypos):
    ...
    if is_optout[ri]:
        ...
        col = FLAT
    cax.text(0.50, y, fmt_delta(dv), va="center", ha="center",
...
textbox(Inches(0.80), Inches(0.40), Inches(12.1), Inches(1.0),
        [[("Julkiset hoivapalvelut ja Attendo johtavat valintahalukkuutta",
           23, PX_INK, True)],
         [("Julkiset hoivapalvelut 37 % ja Attendo 29 % – Attendo nousi "
           "+4 %-yks. edellisestä aallosta",
           13.5, PX_MUTED, False)]],
        space_after=3)

# Section label above chart
textbox(Inches(0.80), Inches(1.52), Inches(9.0), Inches(0.32),
        [[("VALINTA- JA SUOSITTELUHALUKKUUS  ·  osuus arvioineista (%)",
           11, PX_TEAL, True)]])
...
textbox(Inches(0.80), Inches(7.04), Inches(10.2), Inches(0.42),
        [[("Kysymys: ", 9.5, PX_MUTED, True),
          ("”Mitä alla olevista palveluntarjoajista voisit kuvitella "
           "valitsevasi itsellesi tai suosittelisit?”", 9.5, PX_MUTED, False)]])
textbox(Inches(11.0), Inches(7.04), Inches(1.85), Inches(0.42),
        [[("Arvioineet, n ≈ 865", 9.5, PX_MUTED, True)]],
