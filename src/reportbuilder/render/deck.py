...
def _resolve_slot(prs: Presentation, style, slot_name: str,
    ...
    if render_mode == "image":
        ...
        # …but 1.9" is a guess about OUR chrome, and a template can put its
        # title band lower than that. Biocodex's design lives in a LAYOUT (so
        # `harvested_profile` declines it, there being nothing to redraw) and
        # that layout carries no chart placeholder (so `chart_slot` is None and
        # the branch above is skipped) — leaving this guess as the only case
