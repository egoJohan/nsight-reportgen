...
async def oidc_callback(provider: str, request: Request,
    ...
    if isinstance(resolved, identity.SignInRefused):
        ...
        # The standard answer, the same as any other sign-in that did not go
        # through: no account is created, nothing is filed, and nothing tells
        # the visitor whether an account exists for that address. Asking for
        # an account from the login page was removed (Johan, 2026-09-25);
        # people get in by an admin's invitation.
        redirect = Response(status_code=302,
                            headers={"Location": "/login?error=sign_in_failed"})
        redirect.delete_cookie(_OAUTH_COOKIE, path="/auth")
        return redirect
